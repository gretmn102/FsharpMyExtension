...
/// max 2046Mb
/// предполагается, что `Accept-Ranges` доступен.
let download2 (parts:int) createHttp resp =
    let rec read (resp:Net.WebResponse) output (start,stop) =
        use inp = resp.GetResponseStream()
        let read offset count = inp.Read(output, offset, count)
        match readingByRange read start stop with
        | None -> ()
    ...
    let start (resp:System.Net.WebResponse) =
        match resp.Headers.Get "Accept-Ranges" with
        | null -> Left "notAcceptRanges"
        | _ ->
            ...
            Right buff
    start resp
let download3 (parts:int) createHttp =
    let (request:System.Net.HttpWebRequest) = createHttp()
    use resp = request.GetResponse()
    download2 (parts:int) createHttp resp
let test3 () =
    ...
    // 00:00:10.798
    let actual =
        download3 4 (fun () -> System.Net.HttpWebRequest.CreateHttp url )
        |> Either.get
        // downloadDefineLength 4 url |>
    // System.Text.Encoding.UTF8.GetString expect
    // System.Text.Encoding.UTF8.GetString actual
